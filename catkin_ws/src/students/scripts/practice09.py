...
def inverse_kinematics_xyzrpy(x, y, z, roll, pitch, yaw, Ti, Wi):
    ...
    while (numpy.linalg.norm(error) > tolerance) and (iterations < max_iterations):
        ...
        q = q - (numpy.dot(numpy.linalg.pinv(J),error)) #Update q estimation with q = q - pseudo_inverse(J)*error
        q = (q + math.pi) % (2 * math.pi) - math.pi#Ensure all angles q are in [-pi,pi]
